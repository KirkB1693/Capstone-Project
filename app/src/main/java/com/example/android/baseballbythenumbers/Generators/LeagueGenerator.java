...
public class LeagueGenerator {
    ...
    public League generateLeague(String leagueName, boolean useDH, int divisionSize, int numberOfDivisions, int countriesToInclude, int[] teamMakeup) {
        ...
        switch (numberOfDivisions) {
            case 0:
                divisions.add(divisionGenerator.generateDivision(NO_DIVISONS, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                break;
            case 1:
                divisions.add(divisionGenerator.generateDivision(NO_DIVISONS, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                break;
            case 2:
                divisions.add(divisionGenerator.generateDivision(WEST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(EAST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                break;
            case 3:
                divisions.add(divisionGenerator.generateDivision(WEST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(CENTRAL, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(EAST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                break;
            case 4:
                divisions.add(divisionGenerator.generateDivision(WEST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(NORTH, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(SOUTH, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                divisions.add(divisionGenerator.generateDivision(EAST, useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                break;
            default:
                for (int i = 0; i < divisionSize; i++) {
                    divisions.add(divisionGenerator.generateDivision(String.valueOf(i), useDH, divisionSize, teamMakeup, numberOfDivisions, countriesToInclude));
                }
                break;
        }
