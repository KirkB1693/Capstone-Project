...
public class GameSimulator {
    ...
    public void simAtBatWithHumanControl() {
        if (gameNotOver) {
            ...
            if (batter != null && batter.getName() != null) {
    ...
    private void playGame() {
        ...
        while (gameNotOver) {
            ...
            Player batter = lineup.get(currentBatter);
            Player pitcher = defense.get(SCOREKEEPING_PITCHER);
            if (pitcher != null && batter != null && batter.getName() != null) {
                gameLog.append(batter.getName()).append(" : ");
                getBatterStaminaAdjustment(batter);
                getPitcherStaminaAdjustment(pitcher);
            } else {
    ...
    private void processEndOfHalfInning() {
        ...
        if (!(((inningsPlayed > 80 && homeScore > visitorScore) || inningsPlayed > 85) && ifScoreNotTiedAtEndOfInning(inningsPlayed, homeScore, visitorScore, homeTeamFinishedAtBat))) {
            ...
            Player pitcher = defense.get(SCOREKEEPING_PITCHER);
            if (pitcher != null && !isPitcher(pitcher)) {     // if we pinch hit for pitcher with a position player, we need a new pitcher
    ...
    private PitchingLine getPitchingLineForCurrentPitcher() {
        Player pitcher = defense.get(SCOREKEEPING_PITCHER);
        if (pitcher != null) {
            String currentPitcherName = pitcher.getName();
    ...
    private BattingLine getBattingLineForCurrentBatter() {
        Player batter = lineup.get(currentBatter);
        if (batter != null) {
            String currentBatterName = batter.getName();
    ...
    private Player getBestPinchHitter() {
        ...
        if (runnerInScoringPosition()) {
            while (pinchHitterKey != null && pinchHitterKey == -1) {
    ...
    private boolean shouldWeSwitchPitchers(Player currentPitcher) {
        ...
        if (relievers.size() > 0) {
            ...
            if (pitcherStaminaAdjustment > 500) {                      // If pitcher is tired switch pitchers
                ...
                Player pitcher = defense.get(SCOREKEEPING_PITCHER);
                if (pitcher != null) {
                    if (inningsPlayed < 50 && (battingScore - pitchingScore) > 5 && pitcher.getPrimaryPosition() == STARTING_PITCHER) {  // Starter is losing by more than 5 runs before the 5th inning
                        switchPitchers(currentPitcher);
                        didWeSwitch = true;
                    } else if (inningsPlayed >= 50 && (battingScore - pitchingScore) > 2 && pitcher.getPrimaryPosition() == STARTING_PITCHER) {  // Starter is losing by more than 2 runs after the 5th inning
                        switchPitchers(currentPitcher);
                        didWeSwitch = true;
    ...
    private void switchPitchers(Player currentPitcher) {
        ...
        if (relievers.containsKey(relieverChoice)) {
            ...
            Integer ceilingKey = relievers.ceilingKey(0);
            if (ceilingKey != null) {
                relieverChoice = ceilingKey;
            }
            newPitcher = relievers.get(relieverChoice);
            if (newPitcher != null) {
                defenseToModify.put(SCOREKEEPING_PITCHER, newPitcher);
    ...
    private void doPregameSetup() {
        ...
        Player homePitcher = homeDefense.get(SCOREKEEPING_PITCHER);
        if (homePitcher != null) {
            homePitcher.getPitchingStatsForYear(year).incrementGamesStarted();
            homePitcher.getPitchingStatsForYear(year).incrementGames();
            homePitcher.getBattingStatsForYear(year).incrementGames();
        }
        Player visitingPitcher = visitingDefense.get(SCOREKEEPING_PITCHER);
        if (visitingPitcher != null) {
            visitingPitcher.getPitchingStatsForYear(year).incrementGamesStarted();
            visitingPitcher.getPitchingStatsForYear(year).incrementGames();
            visitingPitcher.getBattingStatsForYear(year).incrementGames();
        }

        for (int i = 1; i < 10; i++) {
            Player batter = homeLineup.get(i);
            if (batter != null) {
                if (batter.getPrimaryPosition() != STARTING_PITCHER) {
                    batter.getBattingStatsForYear(year).incrementGames();
                }
            }
            batter = visitingLineup.get(i);
            if (batter != null) {
                if (batter.getPrimaryPosition() != STARTING_PITCHER) {
                    batter.getBattingStatsForYear(year).incrementGames();
                }
    ...
    private void initializeBoxScore(BoxScore boxScore, TreeMap<Integer, Player> lineup, TreeMap<Integer, Player> defense) {
        ...
        Player pitcher = defense.get(SCOREKEEPING_PITCHER);
        if (pitcher != null) {
            boxScore.addPitchingLine(new PitchingLine(boxScore.getBoxScoreId(), 1, pitcher.getName()));
        }
    }
